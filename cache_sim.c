...
int main(int argc, char *argv[]){
	FILE *trace_file = fopen("gcc_trace.txt", "r");
        ...
        initialize_cache(assoc,num_sets,vc_num_blocks,block_size,cache,counters,dirty,vc,vc_counter,vc_dirty);
        ...
        initialize_cache(assoc_l2,num_sets_l2,vc_num_blocks_l2,block_size_l2,cache_,counters_,dirty_,vc_,vc_counter_,vc_dirty_);        
        ...
        while (fscanf(trace_file, "%c %x\n", &operation, &address) != EOF){
       	...
       	L1=cache_block(assoc,num_sets, vc_num_blocks, block_size,operation,address,vc_enable,cache,counters,dirty,vc,vc_counter,vc_dirty) ;
       	...
       	if ((L1.read_miss+L1.write_miss-L1.no_swaps==1 || L1.wb==1) & (size_l2 !=0)){
	       	...
	       	L2=cache_block(assoc_l2,num_sets_l2, vc_num_blocks_l2, block_size_l2,op,address,0,cache_,counters_,dirty_,vc_,vc_counter_,vc_dirty_) ;
